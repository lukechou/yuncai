      ...
      if (data.retCode == 100000) {
        ...
        if (data.retData.data.length > 0) {
          ...
          for (var i = 1; i <= detailData.length; i++) {
            ...
            htmlOutput += '<tr class="dzgd-tr"><td class="j-loty-name" loty-name="' + dataItem.loty_name + '" ldr-id="' + dataItem.ldr_id + '" leader-uid="' + dataItem.leader_uid + '">' + dataItem.loty_cnname + '</td><td>' + dataItem.leader_username + '</td><td>' + dataItem.dateline + '</td><td><span class="fc-3">' + dataItem.follow_value + '</span>元</td><td>' + dataItem.follow_times + '</td><td><button class="btn btn-sear btn-oper j-mod-dzgd">修改</button><a href="javascript:;" class="j-undo">撤销</a></td></tr>' + dzgdTr;
          }
        } else {
  ...
  $(document).delegate('.j-imme-follow', 'click', function(event) {
    ...
    switch (dataType) {
      ...
      case '2':
        ...
        outhtml = '<div class="frbox"><img src="http://static3.yuncai.com/front_images/fail.png" alt="success" class="icon"><div class="text"><p>每次认购比例：<span class="fc-3">' + unitPercentageZ + '</span>%</p><p>认购金额上限：<span class="fc-3">' + maxPrice  + '</span>元</p><p>定制次数：<span class="fc-3">' + maxBuyTimes + '</span></p><p>确认按以上信息进行跟单吗？</p></div></div>';
        break;
    }
