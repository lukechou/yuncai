...
require(['jquery', 'lodash', 'store', 'app', 'PL3', 'bootstrap', 'core'], function($, _, store, APP, PL3) {
	...
	PL3.nav = (function() {
		...
		var nav = {
			...
			toggleTabs: function() {
				...
				PL3.ballAear.find('a').removeClass('active dis')
	...
	function smallToggleTabs(box) {
		...
		} else if (_.indexOf(['d1', 'd2', 'bx', 'qo', 'hs', 'dx', 'tx'], b) >= 0) {
			$('.j-jx-zhus').show();
		} else {
	...
	$(".j-num-group").on('click', 'a', function(event) {
		...
		var arr = null;
		var num = parseInt(APP.filterStr(_this.html()));
		var ballState = _this.hasClass('active');
		...
		if (_.indexOf(['dx', 'qo', 'hs', 'st', 'tlj'], b) >= 0) {

			num = _this.attr('data-new');
		...
		if ((s === 'bx3' || s === 'bx6') && $(this).hasClass('dis')) {
		...
		if (s === 'bx3') {
			...
			if ($('[data-bx=' + num + '].active').length == 2) {

				$('[data-bx=' + num + ']').addClass('dis');
				$('[data-bx=' + num + '].active').removeClass('dis');

			} else {

				$('[data-bx=' + num + ']').removeClass('dis');
		...
		if (s === 'bx6') {
			...
			if ($('[data-bxs=' + num + '].active').length == 1) {

				$('[data-bxs=' + num + ']').addClass('dis');
				$('[data-bxs=' + num + '].active').removeClass('dis');

			} else {

				$('[data-bxs=' + num + ']').removeClass('dis');
	...
	PL3.chooseBuyBtn.on('click', function(event) {
		...
		switch (type) {
			case 0:
				...
				if (_.indexOf(['d1', 'cd1', 'd2', 'cd2', 'bx3', 'bx6', 'dx', 'qo', 'hs', 'st', 'tlj'], s) >= 0) {

					var modifyCode = null;

					for (var i = 0; i < PL3.G_CHOOSE.codes[0].length; i++) {
						_.remove(PL3.G_CHOOSE.codes[0][i], function(n) {
							return n === '_';
						})
					};

					var dataBit = PL3.G_MODIFY_CODE_OBJ.codeKey;

					_.remove(PL3.G_BUY.codes, function(n) {
						return n.key == dataBit;
					});

					$('.br-zhu-item[dataBit=' + dataBit + ']').remove();

					PL3.makeChooseCodeHtml(PL3.G_CHOOSE.codes, true);
			...
			case 1:
				...
				if (PL3.G_CHOOSE.money > 0) {
					...
					bool = PL3.makeChooseCodeHtml(PL3.G_CHOOSE.codes, true);
				}
		...
		if (bool) {
			...
			$('.box-' + b + '-' + s).find('.j-num-group a').removeClass('active dis');
			$('.box-' + b).find('.j-num-group a').removeClass('active');
	...
	function updateZhuMain() {
		...
		if (s === 'cgtz') {
			...
			html += '<span>' + PL3.G_CHOOSE.codes[0][0].join('') + '</span>';

			if (b === 'zx') {
				if (PL3.G_CHOOSE.codes[0][1].length > 0) {
					html += '<span>' + PL3.G_CHOOSE.codes[0][1].join('') + '</span>';
				}

				if (PL3.G_CHOOSE.codes[0][1].length > 0) {
					html += '<span>' + PL3.G_CHOOSE.codes[0][2].join('') + '</span>';
				}
			}
	...
	function getBx3Zhushu(codes) {
		...
		if (codes[0] && codes[1] && codes[2]) {
			...
			for (var i = 0; i < codes[0].length; i++) {
				...
				if (_.indexOf(codes[1], codes[0][i]) >= 0) {

					result += codes[2].length;

				}

				if (_.indexOf(codes[2], codes[0][i]) >= 0) {

					result += codes[1].length;
			...
			for (var i = 0; i < codes[1].length; i++) {
				...
				if (_.indexOf(codes[2], codes[1][i]) >= 0) {

					result += codes[0].length;
	...
	function calculateChooseCodes() {
		...
		if (PL3.G_CHOOSE.codes[0][0]) {
			numAlen = PL3.G_CHOOSE.codes[0][0].length;
		} else {
			numAlen = 0;
		}

		if (PL3.G_CHOOSE.codes[0][1]) {
			numBlen = PL3.G_CHOOSE.codes[0][1].length;
		} else {
			numBlen = 0;
		}

		if (PL3.G_CHOOSE.codes[0][2]) {
			numClen = PL3.G_CHOOSE.codes[0][2].length;
		} else {
			numClen = 0;
		}
		...
		if ((b === 'zx' && s === 'cgtz') || b === 'tx' || s === 'bx6') {
			...
			for (var i = 0, len = PL3.G_CHOOSE.codes.length; i < len; i++) {
				...
				if (numAlen > 0 && numBlen > 0 && numClen > 0) {
		...
		if (b === 'd1') {
			...
			zhushu = numAlen + numBlen + numClen;
		...
		if (s === 'bx3') {
			...
			zhushu = getBx3Zhushu(PL3.G_CHOOSE.codes[0]);
		...
		if (s === 'd2') {
			...
			zhushu = numAlen * numBlen + numAlen * numClen + numBlen * numClen;

		}

		if (s === 'cd2') {

			zhushu = numAlen + QUEUE.getACTotalNum(numAlen, 2, 'C');

		}

		if (b === 'hs') {

			objCount = $('.j-hs-btn.active').length;
			zhushu = zhushu = objCount ? objCount : 0;

		}

		if (b === 'qo') {

			objCount = $('.j-qo-btn.active').length;
			zhushu = zhushu = objCount ? objCount : 0;
		...
		if (b === 'dx') {
			...
			objCount = $('.j-dx-btn.active').length;
			zhushu = zhushu = objCount ? objCount : 0;
		...
		if (b === 'tlj') {
			...
			objCount = $('.j-tlj-btn.active').length;

			zhushu = objCount ? objCount : 0;

		}

		if (b === 'st') {

			objCount = $('.j-st-btn.active').length;
			zhushu = zhushu = objCount ? objCount : 0;
	...
	function reflectChooseCode(buyIndex) {
		...
		// 和数，大小，奇偶修改
		if (_.indexOf(['hs', 'qo', 'dx'], PL3.nav.big) >= 0) {

			$('.box-' + PL3.nav.big + ' .j-num-group a').removeClass('active');

			$('#choose_to_buy').removeAttr("disabled");

			for (var i = 0; i < codes[0].length; i++) {

				$('.j-' + PL3.nav.big + '-btn[data-new=' + codes[0][i] + ']').addClass('active');
		...
		// 包选选号限制
			if (PL3.nav.small === 'bx3') {

				for (var i = 0; i < 10; i++) {

					if ($('[data-bx=' + i + '].active').length == 2) {

						$('[data-bx=' + i + ']').addClass('dis');
						$('[data-bx=' + i + '].active').removeClass('dis');
			...
			if (PL3.nav.small === 'bx6') {

				for (var i = 0; i < 10; i++) {

					if ($('[data-bxs=' + i + '].active').length == 1) {

						$('[data-bxs=' + i + ']').addClass('dis');
						$('[data-bxs=' + i + '].active').removeClass('dis');
