...
$(document).ready(function() {
            ...
            case 0:
                ...
                switch (G_CHOOSE.playType) {
                    ...
                    case CoreJxssc.playType['R2']:
                        var wanWeiHtml = (G_CHOOSE.codes[0]) ? G_CHOOSE.codes[0].join('') : '-';
                        var qianWeiHtml = (G_CHOOSE.codes[1]) ? G_CHOOSE.codes[1].join('') : '-';
                        var baiWeiHtml = (G_CHOOSE.codes[2]) ? G_CHOOSE.codes[2].join('') : '-';
                        var shiWeiHtml = (G_CHOOSE.codes[3]) ? G_CHOOSE.codes[3].join('') : '-';
                        var geWeiHtml = (G_CHOOSE.codes[4]) ? G_CHOOSE.codes[4].join('') : '-';
                        html =
                            '<div class="br-zhu-item clearfix" databit="' + G_MODIFY_CODE_OBJ.codeKey + '">\
                            <b>[' + CoreJxssc.playCNType[G_CHOOSE.playType] + ']</b>\
    ...
    function makeChooseCodeHtml(playType, codes) {
        ...
        if ((G_BUY.codes.length + G_CHOOSE.codes.length) > CoreJxssc.maxBuyCodeLength) {
            APP.showTips('您的投注号码多于' + CoreJxssc.maxBuyCodeLength + '行，请返回重新选择');
            return false;
        }
        var newCodes = codes;
        ...
        switch (playType) {
            ...
            case CoreJxssc.playType['R1']:
            case CoreJxssc.playType['R2']:
                var wanWeiHtml = (newCodes[0]) ? newCodes[0].join('') : '-';
                var qianWeiHtml = (newCodes[1]) ? newCodes[1].join('') : '-';
                var baiWeiHtml = (newCodes[2]) ? newCodes[2].join('') : '-';
                var shiWeiHtml = (newCodes[3]) ? newCodes[3].join('') : '-';
                var geWeiHtml = (newCodes[4]) ? newCodes[4].join('') : '-';
                var money = 2 * CoreJxssc.getBetNum(playType, newCodes);
                html += '<div class="br-zhu-item clearfix" dataBit=' + G_BUY.rowIndex + '>';
                html += '<b>[' + CoreJxssc.playCNType[playType] + ']</b>';
                html += '<div class="list">';
    ...
    var fnBuy = function() {
        ...
        for (var i = 0; i < G_BUY.codes.length; i++) {
            ...
            switch (G_BUY.codes[i].playType) {
                ...
                case CoreJxssc.playType['R1']:
                case CoreJxssc.playType['R2']:
                    var WanWeiCodes = (G_BUY.codes[i].value[0]) ? G_BUY.codes[i].value[0].join('') : '-';
                    var QianWeiCodes = (G_BUY.codes[i].value[1]) ? G_BUY.codes[i].value[1].join('') : '-';
